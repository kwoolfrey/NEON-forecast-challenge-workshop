...
###This model uses forecasts of air temperature and shortwave infrared, provided by NOAA, to predict water temperature for several lakes monitored by
### the NEON program. 

# Add a brief description of your modeling approach

### This model is a simple regression model using air temperature and the 3 day SWIR average, in addition to the previous day's water temperature,
### to predict water temperature. Complexity was added by accounting for uncertainty, as described below. 
...
### equal to the standard deviation value. Initial condition uncertainty was also added for the 1 day water temperature lag, by using 
### the original water temperature as the mean and 0.1 as the standard deviation for a random draw. The resulting forecast has much 
### more uncertainty than the original, reflecting the uncertainty in our drivers and parameters, as well as the overall process and our initial conditions. 
...
weather_future_daily <- weather_future_daily %>% group_by(datetime) %>%
  mutate(avg_3day_rad = rollmean(surface_downwelling_shortwave_flux_in_air, k = 5, align = "right", fill = NA))
...
#   i = 1 
  ...
  fit <- lm(site_target$temperature ~ site_target$air_temperature + site_target$avg_3day_rad + site_target$lagged)
